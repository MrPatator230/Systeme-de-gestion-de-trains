...
export default function Horaires() {
  ...
  const handleSubmit = (e) => {
    ...
    // Validate served stations: all must have non-empty name, arrivalTime, and departureTime
    const servedStationsList = servedStations.filter(s => s.name.trim() !== '');
    if (servedStationsList.length === 0) {
      alert('La liste des gares desservies ne peut pas être vide.');
      return;
    }
    for (const station of servedStationsList) {
      if (!station.name.trim() || !station.arrivalTime || !station.departureTime) {
        alert('Chaque gare desservie doit avoir un nom, une heure d\'arrivée et une heure de départ valides.');
        return;
      }
    }
